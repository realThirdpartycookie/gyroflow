...
[ "_main", "_gf_export_chunk", "_gf_export_config", "_gf_export_done", "_gf_export_error", "_gf_export_frame_params", "_gf_export_rendered", "_gf_web_files_picked", "_FnBoxWrapper_destructor", "_FnBoxWrapper_operator", "_ProcessQtEvent", "_QRustClosureSlotObject_call", "_QRustClosureSlotObject_destruct", "_QtDestroyFuture", "_RustObject_destruct", "_RustObject_metaObject", "_Rust_QAbstractItemModel_columnCount", "_Rust_QAbstractItemModel_data", "_Rust_QAbstractItemModel_index", "_Rust_QAbstractItemModel_parent", "_Rust_QAbstractItemModel_roleNames", "_Rust_QAbstractItemModel_rowCount", "_Rust_QAbstractItemModel_setData", "_Rust_QAbstractListModel_data", "_Rust_QAbstractListModel_roleNames", "_Rust_QAbstractListModel_rowCount", "_Rust_QAbstractListModel_setData", "_Rust_QAbstractTableModel_columnCount", "_Rust_QAbstractTableModel_data", "_Rust_QAbstractTableModel_roleNames", "_Rust_QAbstractTableModel_rowCount", "_Rust_QAbstractTableModel_setData", "_Rust_QQmlExtensionPlugin_registerTypes", "_Rust_QQuickItem_classBegin", "_Rust_QQuickItem_componentComplete", "_Rust_QQuickItem_geometryChanged", "_Rust_QQuickItem_mousePressEvent", "_Rust_QQuickItem_releaseResources", "_Rust_QQuickItem_updatePaintNode", "_Rust_QQuickPaintedItem_classBegin", "_Rust_QQuickPaintedItem_componentComplete", "_Rust_QQuickPaintedItem_geometryChanged", "_Rust_QQuickPaintedItem_mousePressEvent", "_Rust_QQuickPaintedItem_paint", "_Rust_QQuickPaintedItem_releaseResources", "_Rust_QmlEngineHolder_ctor", "_Rust_QmlEngineHolder_dtor", "_Rust_MDKPlayerItem_processPixels", "_Rust_MDKPlayerItem_processTexture", "_Rust_MDKPlayerItem_readyForProcessing", "_qvr_web_open_decoder", "_qvr_web_probe", "__emscripten_thread_init", "__emscripten_thread_exit", "__emscripten_thread_crashed", "__emscripten_thread_mailbox_await", "__emscripten_tls_init", "_pthread_self", "checkMailbox", "__embind_initialize_bindings", "establishStackSpace", "invokeEntryPoint", "PThread", "___indirect_function_table", "_fflush", "_jsHaveAsyncify", "_jsHaveJspi", "___asyncjs__qt_jspi_suspend_js", "_qt_jspi_resume_js", "_qt_jspi_can_resume_js", "_init_jspi_support_js", "_qt_asyncify_suspend_js", "_qt_asyncify_resume_js", "___start_em_js", "___stop_em_js", "onRuntimeInitialized" ].forEach(prop => {
...
var PThread = {
 ...
 initMainThread() {
  var pthreadPoolSize = navigator.hardwareConcurrency * 2 + 24;
 ...
 allocateUnusedWorker() {
  var worker;
  var pthreadMainJs = locateFile("gyroflow.worker.js");
...
function ___syscall_pipe(fdPtr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(25, 1, fdPtr);
 fdPtr >>>= 0;
 try {
  if (fdPtr == 0) {
...
function ___syscall_poll(fds, nfds, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(26, 1, fds, nfds, timeout);
 fds >>>= 0;
 try {
  var nonzero = 0;
...
function ___syscall_symlink(target, linkpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 1, target, linkpath);
 target >>>= 0;
 linkpath >>>= 0;
 try {
...
function ___syscall_truncate64(path, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(36, 1, path, length);
 path >>>= 0;
 length = bigintToI53Checked(length);
 try {
  if (isNaN(length)) return 61;
...
function ___syscall_utimensat(dirfd, path, times, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(38, 1, dirfd, path, times, flags);
 path >>>= 0;
 times >>>= 0;
 try {
  path = SYSCALLS.getStr(path);
...
function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(40, 1, len, prot, flags, fd, offset, allocated, addr);
 len >>>= 0;
...
function _emscripten_get_element_css_size(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(42, 1, target, width, height);
 target >>>= 0;
 width >>>= 0;
 height >>>= 0;
...
var GFWEB = {
 ...
 exports: {},
 gpuDevice() {
  return GFWEB.device ??= navigator.gpu.requestAdapter({
 ...
 undistort(d, code, coeffs) {
  var module = d.createShaderModule({
   code: code
  });
  var ro = {
   type: "read-only-storage"
  }, F = GPUShaderStage.FRAGMENT;
  var layout = d.createBindGroupLayout({
   entries: [ {
    binding: 0,
    visibility: F,
    buffer: {
  ...
  var buf = (size, usage) => d.createBuffer({
   size: Math.max(16, Math.ceil(size / 16) * 16),
   usage: usage | GPUBufferUsage.COPY_DST
  });
  var b = {
   coeffs: buf(coeffs.byteLength, GPUBufferUsage.STORAGE),
   drawing: buf(16, GPUBufferUsage.STORAGE)
  };
  d.queue.writeBuffer(b.coeffs, 0, coeffs);
  var pipelines = {}, tex = null, bind = null;
  var ensure = (k, size, usage) => {
   if (!b[k] || b[k].size < size) {
    b[k]?.destroy();
    b[k] = buf(size, usage);
    bind = null;
   }
  ...
  return {
   ...
   render(source, w, h, params, matrices, mesh, view) {
    ...
    bind ??= d.createBindGroup({
     layout: layout,
     entries: [ ...[ "params", "matrices", "coeffs", "mesh", "drawing" ].map((k, i) => ({
      binding: i,
      resource: {
       buffer: b[k]
      }
    ...
    pipelines[key] ??= d.createRenderPipeline({
     ...
     vertex: {
      module: module,
      entryPoint: "undistort_vertex"
     },
     fragment: {
      module: module,
      entryPoint: "undistort_fragment",
      targets: [ {
       format: "rgba8unorm"
      } ],
      constants: {
 ...
 addFiles(list) {
  ...
  return Array.from(list).map(f => {
   GFWEB.createNode(dir, f.name, GFWEB.FILE_MODE, f);
   return "/web/" + n + "/" + f.name;
  });
 },
 fileByPath(path) {
  try {
   return GFWEB.files[FS.lookupPath(path).node.fileId] || null;
  } catch (e) {
   return null;
  }
 },
 node_ops: {
  ...
  readdir(node) {
   return [ ".", ".." ].concat(Object.keys(node.contents));
  },
  symlink(parent, newName, oldPath) {
   throw new FS.ErrnoError(63);
  }
 ...
 stream_ops: {
  ...
  llseek(stream, offset, whence) {
   var position = offset;
   if (whence === 1) position += stream.position; else if (whence === 2) position += stream.node.size;
 ...
 startReader() {
  ...
  GFWEB.reader = new Worker(URL.createObjectURL(new Blob([ src ], {
 ...
 readChunk(id, position, length) {
  ...
  Atomics.store(c, 0, 0);
  Atomics.notify(c, 0);
  if (n < 0) throw new FS.ErrnoError(29);
  return n;
 }
};

var _gf_dec_close = function(id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(54, 0, id);
...
function _gf_dec_open(id, pathPtr, codecPtr, descPtr, descLen, codedW, codedH, samplesPtr, nSamples, rangesPtr, nRanges, mode, outW, outH, backlogPtr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(55, 1, id, pathPtr, codecPtr, descPtr, descLen, codedW, codedH, samplesPtr, nSamples, rangesPtr, nRanges, mode, outW, outH, backlogPtr);
 var file = GFWEB.fileByPath(UTF8ToString(pathPtr));
 var samples = GROWABLE_HEAP_F64().slice(samplesPtr >> 3, (samplesPtr >> 3) + nSamples * 4);
 var ranges = [];
 for (var i = 0; i < nRanges; i++) ranges.push([ GROWABLE_HEAP_F64()[(rangesPtr >> 3) + i * 2 >>> 0], GROWABLE_HEAP_F64()[(rangesPtr >> 3) + i * 2 + 1 >>> 0] ]);
 ...
 s.decoder = new VideoDecoder({
  output: f => {
   if (s.closed || !inRange(f.timestamp)) {
    f.close();
    kick();
    return;
   }
   ...
   GROWABLE_HEAP_U8().set(img, ptr >>> 0);
   deliver(f.timestamp, canvas.width, canvas.height, ptr, img.length);
   kick();
  },
  error: e => {
   s.error = e;
   console.error("gfweb decoder:", e);
   kick();
  }
 });
 s.decoder.configure({
  codec: UTF8ToString(codecPtr),
  description: GROWABLE_HEAP_U8().slice(descPtr, descPtr + descLen),
  codedWidth: codedW,
  codedHeight: codedH,
 ...
 for (var [a, b] of ranges) {
  ...
  for (var i = start; i < n; i++) {
   if (samples[i * 4 + 3] && samples[i * 4 + 2] > b) break;
   plan.push(i);
  }
 }
 console.log(`gfweb: decode ${plan.length}/${n} samples, ranges ${JSON.stringify(ranges.map(r => r.map(x => isFinite(x) ? Math.round(x / 1e3) : x)))} ms, mode ${mode}, ${outW}x${outH}`);
 var busy = () => s.decoder.decodeQueueSize > 4 || (mode == 1 ? s.frames.length > 6 : GROWABLE_HEAP_I32()[backlogPtr >>> 2] > 6);
 (async () => {
  try {
   for (var i of plan) {
    while (!s.closed && !s.error && busy()) await new Promise(r => {
     s.wake = r;
     setTimeout(r, 20);
    });
    ...
    s.decoder.decode(new EncodedVideoChunk({
     type: samples[i * 4 + 3] ? "key" : "delta",
     timestamp: samples[i * 4 + 2],
     data: data
    }));
   }
...
function _gf_export_cancel(job) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(56, 0, job);
...
var _gf_export_flush = job => {
 var e = GFWEB.exports[job];
 if (!e) return;
 var wait = () => e.ready || e.failed ? Promise.resolve() : new Promise(r => setTimeout(() => wait().then(r), 20));
 wait().then(() => e.failed ? null : e.encoder.flush()).then(() => {
  if (!e.failed) _gf_export_done(job);
 }, err => {
  console.error(err);
 });
};

var _gf_export_run = function(job, shaderPtr, coeffsPtr, nCoeffs, inW, inH, outW, outH, frameDur) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(57, 1, job, shaderPtr, coeffsPtr, nCoeffs, inW, inH, outW, outH, frameDur);
 var e = GFWEB.exports[job], s = GFWEB.decoders[job];
 var code = UTF8ToString(shaderPtr).replace(/@fragment var/g, "var");
 var coeffs = GROWABLE_HEAP_F32().slice(coeffsPtr >> 2, (coeffsPtr >> 2) + nCoeffs);
 ...
 (async () => {
  try {
   ...
   var src = new OffscreenCanvas(inW, inH), src2d = src.getContext("2d", {
    alpha: false
   });
   ...
   for (;;) {
    while (!s.frames.length && !s.done && !s.error && !e.failed) await new Promise(r => {
     s.consumer = r;
     setTimeout(r, 50);
    });
    if (e.failed) return;
    ...
    s.wake?.();
    if (!f) break;
    if (!_gf_export_frame_params(job, f.timestamp, paramsOut)) {
     f.close();
     return fail();
    }
    var p = GROWABLE_HEAP_U32().subarray(paramsOut >>> 2, (paramsOut >> 2) + 6 >>> 0);
    src2d.drawImage(f, 0, 0, inW, inH);
    f.close();
    u.render(src, inW, inH, GROWABLE_HEAP_U8().slice(p[0], p[0] + p[1]), GROWABLE_HEAP_U8().slice(p[2], p[2] + p[3]), GROWABLE_HEAP_U8().slice(p[4], p[4] + p[5]), ctx.getCurrentTexture().createView());
    var vf = new VideoFrame(out, {
     timestamp: Math.round(n * frameDur),
     duration: Math.round(frameDur)
    });
    n++;
    while (e.encoder.state == "configured" && e.encoder.encodeQueueSize > 4) await new Promise(r => e.encoder.addEventListener("dequeue", r, {
     once: true
    }));
    if (e.ready) GFWEB.encode(e, vf); else e.pending.push(vf);
    _gf_export_rendered(job, 0, 0);
   }
   console.log(`gfweb: processed ${n} frames in ${((performance.now() - t0) / 1e3).toFixed(1)} s (${(n / (performance.now() - t0) * 1e3).toFixed(1)} fps)`);
   _gf_export_flush(job);
   _gf_export_rendered(job, 1, 0);
  } catch (err) {
   console.error("gfweb export:", err);
   fail();
  }
 })();
};

function _gf_export_start(job, cfgPtr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(58, 1, job, cfgPtr);
 ...
 var fail = msg => {
  if (e.failed) return;
  e.failed = true;
  console.error("gfweb export:", msg);
  var p = stringToNewUTF8(String(msg));
  _gf_export_error(job, p);
 };
 e.encoder = new VideoEncoder({
  output: (chunk, meta) => {
   ...
   if (desc) {
    ...
    GROWABLE_HEAP_U8().set(d, p >>> 0);
    _gf_export_config(job, p, d.length);
   }
 ...
 var candidates = cfg.family == "hevc" ? [ "hvc1.1.6.L156.B0", "hvc1.1.6.L153.B0", "hvc1.1.6.L150.B0", "hvc1.1.6.L123.B0" ] : [ "avc1.640034", "avc1.640033", "avc1.640032", "avc1.640028" ];
 (async () => {
  for (var hw of [ "prefer-hardware", "no-preference" ]) for (var codec of candidates) {
   ...
   try {
    if (!(await VideoEncoder.isConfigSupported(c)).supported) continue;
   } catch (err) {
    continue;
   }
...
function _gf_settings_load() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(59, 1);
 try {
  const s = localStorage.getItem("gyroflow-settings");
...
function _gf_settings_save(ptr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(60, 1, ptr);
 try {
  localStorage.setItem("gyroflow-settings", UTF8ToString(ptr));
 } catch (e) {
  console.warn("settings not saved", e);
 }
}

function _gf_sink_finish(job, hPtr, hLen, mPtr, mLen, namePtr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(61, 1, job, hPtr, hLen, mPtr, mLen, namePtr);
 var e = GFWEB.exports[job];
 if (!e) return;
 var blob = new Blob([ GROWABLE_HEAP_U8().slice(hPtr, hPtr + hLen), ...e.parts, GROWABLE_HEAP_U8().slice(mPtr, mPtr + mLen) ], {
  type: "video/mp4"
 });
 delete GFWEB.exports[job];
...
function _gf_sink_write(job, ptr, len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(62, 1, job, ptr, len);
 GFWEB.exports[job]?.parts.push(GROWABLE_HEAP_U8().slice(ptr, ptr + len));
}

var _gf_video_create = owner => {
 ...
 var ev = (type, a) => {
  if (p.owner) _qvr_web_event(p.owner, type, a || 0);
 };
 var onFrame = (now, md) => {
  p.time = md.mediaTime;
  if (p.to > p.from && md.mediaTime * 1e3 >= p.to) v.currentTime = p.from / 1e3;
  ev(1, md.mediaTime);
...
var _gf_video_pause = h => GFWEB.players[h].v.pause();

var _gf_video_play = h => {
 GFWEB.players[h].v.play().catch(e => console.warn("video play:", e.message));
};

var _gf_video_seek = (h, ms, exact) => {
 var v = GFWEB.players[h].v;
 if (!exact && v.fastSeek) v.fastSeek(ms / 1e3); else v.currentTime = ms / 1e3;
};

var _gf_video_set = (h, what, val) => {
 var v = GFWEB.players[h].v;
 if (what == 0) v.muted = !!val; else if (what == 1) v.volume = Math.max(0, Math.min(1, val)); else if (what == 2) v.playbackRate = val;
};

var _gf_video_set_range = (h, from, to) => {
 var p = GFWEB.players[h];
 p.from = from;
 p.to = to;
};

var _gf_video_set_url = (h, path) => {
...
var _gf_video_upload = (h, tex) => {
 ...
 if (p.rot) {
  if (!p.cv || p.cv.width != p.tw || p.cv.height != p.th) {
   p.cv = new OffscreenCanvas(p.tw, p.th);
...
var _gf_web_download = function(namePtr, ptr, len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(63, 1, namePtr, ptr, len);
 var a = document.createElement("a");
 a.href = URL.createObjectURL(new Blob([ GROWABLE_HEAP_U8().slice(ptr, ptr + len) ]));
 a.download = UTF8ToString(namePtr);
 document.body.appendChild(a);
 a.click();
 a.remove();
...
var _gf_web_pick_files = (accept, multiple, cbId) => {
 ...
 var done = paths => {
  ...
  _gf_web_files_picked(cbId, stringToNewUTF8(JSON.stringify(paths)));
 };
...
globalThis.gfWebAddFiles = files => GFWEB.addFiles(files);

globalThis.gfHeapMB = () => Math.round(GROWABLE_HEAP_U8().length / 1048576);
...
var wasmImports = {
 ...
 /** @export */ gf_dec_close: _gf_dec_close,
 /** @export */ gf_dec_open: _gf_dec_open,
 /** @export */ gf_export_cancel: _gf_export_cancel,
 /** @export */ gf_export_run: _gf_export_run,
 /** @export */ gf_export_start: _gf_export_start,
 /** @export */ gf_settings_load: _gf_settings_load,
 /** @export */ gf_settings_save: _gf_settings_save,
 /** @export */ gf_sink_finish: _gf_sink_finish,
 /** @export */ gf_sink_write: _gf_sink_write,
 /** @export */ gf_video_create: _gf_video_create,
 /** @export */ gf_video_destroy: _gf_video_destroy,
 /** @export */ gf_video_get: _gf_video_get,
 /** @export */ gf_video_pause: _gf_video_pause,
 /** @export */ gf_video_play: _gf_video_play,
 /** @export */ gf_video_seek: _gf_video_seek,
 /** @export */ gf_video_set: _gf_video_set,
 /** @export */ gf_video_set_range: _gf_video_set_range,
 /** @export */ gf_video_set_url: _gf_video_set_url,
 /** @export */ gf_video_upload: _gf_video_upload,
 /** @export */ gf_web_download: _gf_web_download,
 /** @export */ gf_web_pick_files: _gf_web_pick_files,
...
var _gf_export_chunk = Module["_gf_export_chunk"] = createExportWrapper("gf_export_chunk");

var _gf_export_config = Module["_gf_export_config"] = createExportWrapper("gf_export_config");

var _free = createExportWrapper("free");

var _gf_export_done = Module["_gf_export_done"] = createExportWrapper("gf_export_done");

var _gf_export_error = Module["_gf_export_error"] = createExportWrapper("gf_export_error");

var _gf_export_frame_params = Module["_gf_export_frame_params"] = createExportWrapper("gf_export_frame_params");

var _gf_export_rendered = Module["_gf_export_rendered"] = createExportWrapper("gf_export_rendered");

var _gf_web_files_picked = Module["_gf_web_files_picked"] = createExportWrapper("gf_web_files_picked");
...
var _Rust_MDKPlayerItem_processPixels = Module["_Rust_MDKPlayerItem_processPixels"] = createExportWrapper("Rust_MDKPlayerItem_processPixels");

var _Rust_MDKPlayerItem_processTexture = Module["_Rust_MDKPlayerItem_processTexture"] = createExportWrapper("Rust_MDKPlayerItem_processTexture");

var _Rust_MDKPlayerItem_readyForProcessing = Module["_Rust_MDKPlayerItem_readyForProcessing"] = createExportWrapper("Rust_MDKPlayerItem_readyForProcessing");

var _qvr_web_open_decoder = Module["_qvr_web_open_decoder"] = createExportWrapper("qvr_web_open_decoder");

var _qvr_web_probe = Module["_qvr_web_probe"] = createExportWrapper("qvr_web_probe");

var _qvr_dec_frame = createExportWrapper("qvr_dec_frame");

var _qvr_web_event = createExportWrapper("qvr_web_event");

var _FnBoxWrapper_destructor = Module["_FnBoxWrapper_destructor"] = createExportWrapper("FnBoxWrapper_destructor");

var _FnBoxWrapper_operator = Module["_FnBoxWrapper_operator"] = createExportWrapper("FnBoxWrapper_operator");

var _ProcessQtEvent = Module["_ProcessQtEvent"] = createExportWrapper("ProcessQtEvent");

var _QRustClosureSlotObject_call = Module["_QRustClosureSlotObject_call"] = createExportWrapper("QRustClosureSlotObject_call");

var _RustObject_destruct = Module["_RustObject_destruct"] = createExportWrapper("RustObject_destruct");

var _RustObject_metaObject = Module["_RustObject_metaObject"] = createExportWrapper("RustObject_metaObject");

var _Rust_QAbstractItemModel_columnCount = Module["_Rust_QAbstractItemModel_columnCount"] = createExportWrapper("Rust_QAbstractItemModel_columnCount");

var _Rust_QAbstractItemModel_data = Module["_Rust_QAbstractItemModel_data"] = createExportWrapper("Rust_QAbstractItemModel_data");

var _Rust_QAbstractItemModel_index = Module["_Rust_QAbstractItemModel_index"] = createExportWrapper("Rust_QAbstractItemModel_index");

var _Rust_QAbstractItemModel_parent = Module["_Rust_QAbstractItemModel_parent"] = createExportWrapper("Rust_QAbstractItemModel_parent");

var _Rust_QAbstractItemModel_roleNames = Module["_Rust_QAbstractItemModel_roleNames"] = createExportWrapper("Rust_QAbstractItemModel_roleNames");

var _Rust_QAbstractItemModel_rowCount = Module["_Rust_QAbstractItemModel_rowCount"] = createExportWrapper("Rust_QAbstractItemModel_rowCount");

var _Rust_QAbstractItemModel_setData = Module["_Rust_QAbstractItemModel_setData"] = createExportWrapper("Rust_QAbstractItemModel_setData");

var _Rust_QAbstractListModel_data = Module["_Rust_QAbstractListModel_data"] = createExportWrapper("Rust_QAbstractListModel_data");

var _Rust_QAbstractListModel_roleNames = Module["_Rust_QAbstractListModel_roleNames"] = createExportWrapper("Rust_QAbstractListModel_roleNames");

var _Rust_QAbstractListModel_rowCount = Module["_Rust_QAbstractListModel_rowCount"] = createExportWrapper("Rust_QAbstractListModel_rowCount");

var _Rust_QAbstractListModel_setData = Module["_Rust_QAbstractListModel_setData"] = createExportWrapper("Rust_QAbstractListModel_setData");

var _Rust_QAbstractTableModel_columnCount = Module["_Rust_QAbstractTableModel_columnCount"] = createExportWrapper("Rust_QAbstractTableModel_columnCount");

var _Rust_QAbstractTableModel_data = Module["_Rust_QAbstractTableModel_data"] = createExportWrapper("Rust_QAbstractTableModel_data");

var _Rust_QAbstractTableModel_roleNames = Module["_Rust_QAbstractTableModel_roleNames"] = createExportWrapper("Rust_QAbstractTableModel_roleNames");

var _Rust_QAbstractTableModel_setData = Module["_Rust_QAbstractTableModel_setData"] = createExportWrapper("Rust_QAbstractTableModel_setData");

var _Rust_QQmlExtensionPlugin_registerTypes = Module["_Rust_QQmlExtensionPlugin_registerTypes"] = createExportWrapper("Rust_QQmlExtensionPlugin_registerTypes");

var _Rust_QQuickItem_classBegin = Module["_Rust_QQuickItem_classBegin"] = createExportWrapper("Rust_QQuickItem_classBegin");

var _Rust_QQuickItem_componentComplete = Module["_Rust_QQuickItem_componentComplete"] = createExportWrapper("Rust_QQuickItem_componentComplete");

var _Rust_QQuickItem_geometryChanged = Module["_Rust_QQuickItem_geometryChanged"] = createExportWrapper("Rust_QQuickItem_geometryChanged");

var _Rust_QQuickItem_mousePressEvent = Module["_Rust_QQuickItem_mousePressEvent"] = createExportWrapper("Rust_QQuickItem_mousePressEvent");

var _Rust_QQuickItem_releaseResources = Module["_Rust_QQuickItem_releaseResources"] = createExportWrapper("Rust_QQuickItem_releaseResources");

var _Rust_QQuickItem_updatePaintNode = Module["_Rust_QQuickItem_updatePaintNode"] = createExportWrapper("Rust_QQuickItem_updatePaintNode");

var _Rust_QQuickPaintedItem_paint = Module["_Rust_QQuickPaintedItem_paint"] = createExportWrapper("Rust_QQuickPaintedItem_paint");

var _Rust_QmlEngineHolder_ctor = Module["_Rust_QmlEngineHolder_ctor"] = createExportWrapper("Rust_QmlEngineHolder_ctor");

var _Rust_QmlEngineHolder_dtor = Module["_Rust_QmlEngineHolder_dtor"] = createExportWrapper("Rust_QmlEngineHolder_dtor");

var _Rust_QQuickPaintedItem_mousePressEvent = Module["_Rust_QQuickPaintedItem_mousePressEvent"] = createExportWrapper("Rust_QQuickPaintedItem_mousePressEvent");

var _Rust_QQuickPaintedItem_classBegin = Module["_Rust_QQuickPaintedItem_classBegin"] = createExportWrapper("Rust_QQuickPaintedItem_classBegin");

var _Rust_QQuickPaintedItem_componentComplete = Module["_Rust_QQuickPaintedItem_componentComplete"] = createExportWrapper("Rust_QQuickPaintedItem_componentComplete");

var _Rust_QQuickPaintedItem_releaseResources = Module["_Rust_QQuickPaintedItem_releaseResources"] = createExportWrapper("Rust_QQuickPaintedItem_releaseResources");

var _QRustClosureSlotObject_destruct = Module["_QRustClosureSlotObject_destruct"] = createExportWrapper("QRustClosureSlotObject_destruct");

var _QtDestroyFuture = Module["_QtDestroyFuture"] = createExportWrapper("QtDestroyFuture");

var _Rust_QQuickPaintedItem_geometryChanged = Module["_Rust_QQuickPaintedItem_geometryChanged"] = createExportWrapper("Rust_QQuickPaintedItem_geometryChanged");

var _Rust_QAbstractTableModel_rowCount = Module["_Rust_QAbstractTableModel_rowCount"] = createExportWrapper("Rust_QAbstractTableModel_rowCount");
...
var ___start_em_js = Module["___start_em_js"] = 13872904;

var ___stop_em_js = Module["___stop_em_js"] = 13873994;
...
if (typeof exports === 'object' && typeof module === 'object')
  module.exports = gyroflow_entry;
else if (typeof define === 'function' && define['amd'])
  define([], () => gyroflow_entry);
