...
fn main() {
    ...
    println!("cargo:rerun-if-changed=src/cpp/MDKPlayer_web.cpp");
    ...
    if target_os == "emscripten" { config.include(&qt_include_path).include("src/cpp/web").build("src/lib.rs"); return; } // no MDK for wasm: <video> backend (MDKPlayer_web.cpp)
