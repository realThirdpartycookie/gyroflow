    ...
    loop {
        ...
        let fn_name = match t {
            ExpandSubMacroType::Lit => {
                // wasm-ld rejects `void f()` vs the real Rust signature. argument_helper/return_helper
                // make every parameter pointer-sized, so declare the arity with `const void*`.
                let n = rust_invocation.arguments.len() + rust_invocation.return_type.is_some() as usize;
                let ret = if rust_invocation.return_type.is_some() { "void*" } else { "void" };
                extra_decl.push_str(&format!("extern \"C\" {} {}({});\n", ret, rust_invocation.id, vec!["const void*"; n].join(", ")));
        ...
        for attr in &item.attrs {
            match &attr.meta {
                ...
                syn::Meta::NameValue(syn::MetaNameValue {
                    ...
                    // rustc: a #[path] file behaves like mod.rs, its own `mod`/#[path] items resolve next to it
                    let parent = mod_path.parent().map(|x| x.to_owned()).unwrap_or_default();
                    return self
